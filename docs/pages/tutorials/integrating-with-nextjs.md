...
This page explains how to integrate O-LIBRARY-NAME-O with a NextJS app.

:::tip
This is a community-maintained document. Please [ask the community](/resources/community) if you have questions about this integration. You can also [suggest improvements](O-REPO-URL-O/blob/next/docs/tutorials/integrating-with-nextjs.md) to make it better.
:::
...
- Node: 16.13.1
- NextJS: 12.1.6
- O-LIBRARY-NAME-O: 1.1.0

## Instructions

To get started using O-LIBRARY-NAME-O with NextJS, the following packages must be installed.

```bash
yarn add O-PACKAGE-FULL-NAME-O copy-webpack-plugin next-compose-plugins next-transpile-modules
```

### Enabling ESM

Because O-LIBRARY-NAME-O utilizes ESM, we need to modify our `package.json` to support ESM packages. Simply add the following to
...
The next step is to import O-LIBRARY-NAME-O's default theme (stylesheet) in your `_app.js` file:
...
After importing the theme, you'll need to import the JavaScript files for O-LIBRARY-NAME-O. However, this is a bit tricky to do in NextJS thanks to the SSR environment not having any of the required browser APIs to define endpoints.
...
function CustomEls({ URL }) {
  ...
  useLayoutEffect(() => {
    ...
    import('O-PACKAGE-FULL-NAME-O/dist/utilities/base-path').then(({ setBasePath }) => {
      setBasePath(`${URL}/static/static`);

      // This imports all components
      import('O-PACKAGE-FULL-NAME-O/dist/O-PACKAGE-NAME-O');
      // If you're wanting to selectively import components, replace this line with your own definitions

      // import("O-PACKAGE-FULL-NAME-O/dist/components/button/button");
...
:::tip
This will import all O-LIBRARY-NAME-O components for convenience. To selectively import components, refer to the [Using webpack](/getting-started/installation?id=using-webpack) section of the docs.
:::
...
:::tip
You'll need to set this `BASE_URL` variable inside the build process of whatever local build or CI/CD you have. This will need to be an absolute URL, as a relative URL will cause O-PACKAGE-NAME-O to throw a warning
:::

### webpack Config

Next we need to add O-LIBRARY-NAME-O's assets to the final build output. To do this, modify `next.config.js` to look like this.
...
const withTMCompiled = withTM(['O-PACKAGE-FULL-NAME-O']);

const __dirname = dirname(fileURLToPath(import.meta.url));

export default withPlugins([withTMCompiled], {
  // This is required for ESM to work properly with O-LIBRARY-NAME-O
  ...
  webpack: config => {
    config.plugins.push(
      new CopyPlugin({
        patterns: [
          {
            from: resolve(__dirname, 'node_modules/O-PACKAGE-FULL-NAME-O/dist/assets/icons'),
