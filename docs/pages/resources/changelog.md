...
- Fixed a bug in `<o-tree>` that caused the tree item to collapse when clicking a child item, dragging the mouse, and releasing it on the parent node [#1082]
...
- Fixed a bug in `<o-checkbox>` and `<o-radio>` that caused the checked icons to not scale property when resized
...
- Improved a11y for `<o-input>`, `<o-range>`, `<o-select>`, and `<o-textarea>` so labels and helper text are read properly by screen readers
...
- Updated `<o-spinner>` so the indicator is more obvious
...
- Added `input`, `label`, `prefix`, `clear-button`, `suffix`, `help-text` exported parts to `<o-select>` to make the input customizable
...
- Added `hoist` attribute to `<o-color-picker>`, `<o-dropdown>`, and `<o-select>` to work around panel clipping
- Added `<o-format-bytes>` utility component
- Added `clearable` and `required` props to `<o-select>`
- Added `slclear` event to `<o-input>`
- Added keyboard support to the preview resizer in the docs
- Fixed a bug where the `aria-selected` state was incorrect in `<o-menu-item>`
- Fixed a bug where custom properties applied to `<o-tooltip>` didn't affect show/hide transitions
- Fixed a bug where `--o-input-color-*` custom properties had no effect on `<o-input>` and `<o-textarea>`
- Refactored `<o-dropdown>` and `<o-tooltip>` to use positioner elements so panels/tooltips can be customized easier

## 2.0.0-beta.15

- Added `image-comparer` component
- Added `--width`, `--height`, and `--thumb-size` custom props to `<o-switch>`
- Fixed an `aria-labelledby` attribute typo in a number of components
- Fixed a bug where the `change` event wasn't updating the value in `<o-input>`
- Fixed a bug where `<o-color-picker>` had the wrong border color in the dark theme
- Fixed a bug where `<o-menu-item>` had the wrong color in dark mode when disabled
- Fixed a bug where WebKit's autocomplete styles made inputs looks broken
- Fixed a bug where aria labels were wrong in `<o-select>`
- Fixed a bug where clicking the label wouldn't focus the control in `<o-select>`

## 2.0.0-beta.14

- Added dark theme
- Added `--o-panel-background-color` and `--o-panel-border-color` tokens
- Added `--tabs-border-color` custom property to `<o-tab-group>`
- Added `--track-color` custom property to `<o-range>`
- Added `tag` part to `<o-select>`
...
- Fixed a bug where the text color was being erroneously set in `<o-progress-ring>`
- Fixed a bug where `<o-progress-bar>` used the wrong part name internally for `indicator`
- Removed background color from `<o-menu>`
- Updated to Stencil 1.17.3

## 2.0.0-beta.13

- Added `slactivate` and `sldeactivate` events to `<o-menu-item>`
- Added experimental `<o-animation>` component
...
- Moved scrolling logic from `<o-menu>` to `<o-dropdown>`
...
- Fixed a bug where focus visible observer wasn't removed from `<o-details>`
...
- Replaced make-shift icon buttons with `<o-icon-button>` in alert, dialog, drawer, and tag
...
- Fixed a bug where nested form controls were ignored in `<o-form>`
