...
```jsx react
import { OFilters, OButton } from '%PACKAGE-FULL-PATH%/dist/react';
...
Please check each %LIBRARY-NAME% component documentation used as a filter for more details about the properties that can be passed.
...
Defines the properties of the base filter from where must extend all the filters types. The useful properties are defined to provide auto-complete options for the IDE (i.e.: vscode), the rest are passed to the filter component. Please check each %LIBRARY-NAME% component documentation for more details.
