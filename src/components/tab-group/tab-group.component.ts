 ...
 * @documentation /components/tab-group
 * @status stable
 * @since 1.5
 *
 * @dependency o-icon-button
 *
 * @slot - Used for grouping tab panels in the tab group. Must be `<o-tab-panel>` elements.
 * @slot nav - Used for grouping tabs in the tab group. Must be `<o-tab>` elements.
 *
 * @event {{ name: String }} o-tab-show - Emitted when a tab is shown.
 * @event {{ name: String }} o-tab-hide - Emitted when a tab is hidden.
 *
 ...
 * @csspart scroll-button - The previous/next scroll buttons that show when tabs are scrollable, an `<o-icon-button>`.
...
export default class OTabGroup extends LibraryBaseElement {
  ...
  private activeTab?: OTab;
  private mutationObserver: MutationObserver;
  private resizeObserver: ResizeObserver;
  private tabs: OTab[] = [];
  private panels: OTabPanel[] = [];
  ...
  connectedCallback() {
    const whenAllDefined = Promise.all([
      customElements.whenDefined('o-tab'),
      customElements.whenDefined('o-tab-panel')
    ]);
  ...
  private getAllPanels() {
    return [...this.body.assignedElements()].filter(el => el.tagName.toLowerCase() === 'o-tab-panel') as [OTabPanel];
  }
  ...
  private handleKeyDown(event: KeyboardEvent) {
    ...
    if (['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown', 'Home', 'End'].includes(event.key)) {
      ...
      if (activeEl?.tagName.toLowerCase() === 'o-tab') {
  ...
  private setActiveTab(tab: OTab, options?: { emitEvents?: boolean; scrollBehavior?: 'auto' | 'smooth' }) {
    ...
    if (tab !== this.activeTab && !tab.disabled) {
      ...
      if (options.emitEvents) {
        if (previousTab) {
          this.emit('o-tab-hide', { detail: { name: previousTab.panel } });
        }

        this.emit('o-tab-show', { detail: { name: this.activeTab.panel } });
      }
    }
  }
