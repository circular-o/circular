...
const getPanel = (splitPanel: OSplitPanel, testid: string): HTMLElement => {
  const startPanel = queryByTestId<HTMLElement>(splitPanel, testid);
  expect(startPanel).not.to.be.null;
  return startPanel!;
};

const getPanelWidth = (splitPanel: OSplitPanel, testid: string) => {
  const panel = getPanel(splitPanel, testid);
  const { width } = panel.getBoundingClientRect();
  return width;
};

const getPanelHeight = (splitPanel: OSplitPanel, testid: string) => {
  const panel = getPanel(splitPanel, testid);
  const { height } = panel.getBoundingClientRect();
  return height;
};

const getDivider = (splitPanel: OSplitPanel): Element => {
  const divider = splitPanel.shadowRoot?.querySelector('[part="divider"]');
  expect(divider).not.to.be.null;
  return divider!;
};

describe('<o-split-panel>', () => {
  afterEach(async () => {
    await resetMouse();
  });

  it('should render a component', async () => {
    const splitPanel = await fixture(html` <o-split-panel></o-split-panel> `);

    expect(splitPanel).to.exist;
  });

  it('should be accessible', async () => {
    const splitPanel = await fixture(html`<o-split-panel>
  ...
  describe('panel sizing horizontal', () => {
    it('has two evenly sized panels by default', async () => {
      const splitPanel = await fixture<OSplitPanel>(html`<o-split-panel>
    ...
    it('changes the sizing of the panels based on the position attribute', async () => {
      const splitPanel = await fixture<OSplitPanel>(html`<o-split-panel position="25">
        <div slot="start" data-testid="start-panel">Start</div>
        <div slot="end" data-testid="end-panel">End</div>
      </o-split-panel>`);
    ...
    it('emits the o-reposition	event on position change', async () => {
      const splitPanel = await fixture<OSplitPanel>(html`<o-split-panel>
        <div slot="start">Start</div>
        <div slot="end">End</div>
      </o-split-panel>`);

      const repositionPromise = oneEvent(splitPanel, 'o-reposition');
      splitPanel.position = 10;
    ...
    it('cannot be resized if disabled', async () => {
      const splitPanel = await fixture<OSplitPanel>(html`<o-split-panel disabled>
        <div slot="start">Start</div>
        <div slot="end">End</div>
      </o-split-panel>`);
  ...
  describe('panel sizing vertical', () => {
    it('has two evenly sized panels by default', async () => {
      const splitPanel = await fixture<OSplitPanel>(html`<o-split-panel vertical style="height: 400px;">
        <div slot="start" data-testid="start-panel">Start</div>
        <div slot="end" data-testid="end-panel">End</div>
      </o-split-panel>`);
    ...
    it('changes the sizing of the panels based on the position attribute', async () => {
      const splitPanel = await fixture<OSplitPanel>(html`<o-split-panel position="25" vertical style="height: 400px;">
        <div slot="start" data-testid="start-panel">Start</div>
        <div slot="end" data-testid="end-panel">End</div>
      </o-split-panel>`);
    ...
    it('cannot be resized if disabled', async () => {
      const splitPanel = await fixture<OSplitPanel>(html`<o-split-panel disabled vertical style="height: 400px;">
        <div slot="start">Start</div>
        <div slot="end">End</div>
      </o-split-panel>`);
