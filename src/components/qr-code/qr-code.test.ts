...
import type OQrCode from './qr-code';

const getCanvas = (qrCode: OQrCode): HTMLCanvasElement => {
...
const expectCanvasToHaveAriaLabel = (qrCode: OQrCode, expectedLabel: string): void => {
...
const getQrCodeColors = (qrCode: OQrCode): QrCodeColors => {
  const canvas = getCanvas(qrCode);
...
const expectQrCodeColorsToBe = (qrCode: OQrCode, expectedColors: QrCodeColors): void => {
...
describe('<o-qr-code>', () => {
  it('should render a component', async () => {
    const qrCode = await fixture<OQrCode>(html` <o-qr-code value="test data"></o-qr-code>`);
  ...
  it('uses the label if given', async () => {
    const qrCode = await fixture<OQrCode>(html` <o-qr-code value="test data" label="test label"></o-qr-code>`);

    expectCanvasToHaveAriaLabel(qrCode, 'test label');
  });

  it('sets the correct color for the qr code', async () => {
    const qrCode = await fixture<OQrCode>(html` <o-qr-code value="test data" fill="red"></o-qr-code>`);

    expectQrCodeColorsToBe(qrCode, { foreground: red, background: white });
  });

  it('sets the correct background for the qr code', async () => {
    const qrCode = await fixture<OQrCode>(
      html` <o-qr-code value="test data" fill="red" background="blue"></o-qr-code>`
    );
  ...
  it('has the expected size', async () => {
    const qrCode = await fixture<OQrCode>(html` <o-qr-code value="test data" size="100"></o-qr-code>`);
